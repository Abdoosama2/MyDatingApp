...
12. [Azure Deployment Guide](#azure-deployment-guide)
...
This section outlines how to deploy **MyDatingApp** to Microsoft Azure.

### 1. Database Options on Azure

- **Option A: Azure SQL Database (Recommended for Production)**:
  1. Create an **Azure SQL Database** instance in the Azure Portal.
  2. Change `.UseSqlite(...)` in `Program.cs` to `.UseSqlServer(...)` using the `Microsoft.EntityFrameworkCore.SqlServer` NuGet package.
  3. Set the `ConnectionStrings:Database` App Setting in Azure to your SQL Server connection string.
- **Option B: SQLite on Azure App Service (For Demo/Testing)**:
  - Keep the SQLite provider. Ensure App Service has persistent storage enabled (`WEBSITES_ENABLE_APP_SERVICE_STORAGE = true` in Configuration settings) so the database file `dating.db` is not reset on container restarts.

### 2. Backend Deployment (Azure App Service)

1. **Create App Service**:
   - Create a Web App in Azure (Runtime stack: `.NET 10 (Early Access)` or self-contained publish).
   - Operating System: **Linux** (recommended).
2. **Configure App Settings (Environment Variables)**:
   Navigate to *Settings > Configuration* in the Azure App Service portal and add:
   - `tokenkey` = `YourSuperSecure64CharacterKey`
   - `CloudianrySettings__CloudName` = `YourCloudName`
   - `CloudianrySettings__ApiKey` = `YourApiKey`
   - `CloudianrySettings__ApiSecret` = `YourApiSecret`
   - `ConnectionStrings__Database` = `Data Source=dating.db` *(or your Azure SQL connection string)*
3. **WebSockets Configuration**:
   - In *Configuration > General Settings*, set **WebSockets** to **On** (required for SignalR).
4. **Publish**:
   - Deploy via Git integration, GitHub Actions, or local Git/VS Code publishing.

### 3. Frontend Deployment (Azure Static Web Apps)

1. **Create Static Web App**:
   - Create an **Azure Static Web App** connected to your GitHub repository.
   - Set build presets to **Angular**.
2. **Build Configuration**:
   - App location: `/Api/client`
   - Output location: `dist/client/browser` (or as generated by Angular 21 config).
3. **CORS & API URL Integration**:
   - In the frontend environment configuration, set the production API URL to target your Azure App Service endpoint (e.g., `https://your-api.azurewebsites.net`).
   - Add the Azure Static Web App URL to the CORS allowed origins list in your backend's `Program.cs` or the Azure App Service CORS settings.
